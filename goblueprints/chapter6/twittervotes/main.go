...
func main() {
	var ts struct {
		...
		ConsumeSecret string `env:"SP_TWITTER_SECRET, required"`
		AccessToken   string `env:"SP_TIWTTER_ACCESSTOKEN,required"`
	...
	go func() {
		...
		for {
			...
			if resp.StatusCode != http.StatusOK {
				// this is a nice way to see what the error acutally is:
	...
	// update by forcing the connecting to close
	go func() {
		for {
	...
	<-twitterStopChan // import to avoid panic
	close(votes)
	<-publisherStopChan
}
